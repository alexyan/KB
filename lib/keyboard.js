        ...
        fs.exists(certDir + 'config.json', function (exists) {
            if(exists){
                fs.readFile(certDir + 'config.json', function (err, data) {
                    ...
                    var cmd = process.cwd() + "/shell/enable __user __host".replace(/__host/,data.host).replace(/__user/,data.user);
    ...
    off: function(){
        fs.exists(certDir + 'config.json', function (exists) {
            if(exists){
                fs.readFile(certDir + 'config.json', function (err, data) {
                    data = JSON.parse(data.toString());
                    var cmd = process.cwd() + "/shell/release __user __host".replace(/__host/,data.host).replace(/__user/,data.user);
