        ...
        fs.exists(certDir + 'config.json', function (exists) {
            if(exists){
                fs.readFile(certDir + 'config.json', function (err, data) {
                    ...
                    var cmd = path.join(__dirname, '../shell/','enable') + " __user __host".replace(/__host/,data.host).replace(/__user/,data.user);
    ...
    off: function(){
        fs.exists(certDir + 'config.json', function (exists) {
            if(exists){
                fs.readFile(certDir + 'config.json', function (err, data) {
                    data = JSON.parse(data.toString());
                    var cmd = path.join(__dirname, '../shell/','release') + " __user __host".replace(/__host/,data.host).replace(/__user/,data.user);
                    exec(cmd, function(err,stdout,stderr){
                        if(err){
